...
export class Migration20250623200405 extends Migration {
	override async up(): Promise<void> {
		...
		this.addSql(
			`create table if not exists "contact_email" ("id" text not null, "contact_id" text not null, "email" text not null, "label" text null, "type" text null, "is_active" boolean not null default true, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "contact_email_pkey" primary key ("id"));`,
		);
		this.addSql(
			`CREATE INDEX IF NOT EXISTS "IDX_contact_email_deleted_at" ON "contact_email" (deleted_at) WHERE deleted_at IS NULL;`,
		);

		this.addSql(
			`create table if not exists "contact_phone" ("id" text not null, "contact_id" text not null, "number" text not null, "label" text null, "type" text null, "is_active" boolean not null default true, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "contact_phone_pkey" primary key ("id"));`,
		);
		this.addSql(
			`CREATE INDEX IF NOT EXISTS "IDX_contact_phone_deleted_at" ON "contact_phone" (deleted_at) WHERE deleted_at IS NULL;`,
		);
		...
		this.addSql(
			`create table if not exists "supplier" ("id" text not null, "supplier_number" text null, "customer_number" text null, "internal_key" text null, "company" text not null, "company_addition" text null, "vat_id" text null, "status" text null default 'active', "is_active" boolean not null default true, "language" text null default 'Deutsch', "lead_time" integer null, "website" text null, "note" text null, "bank_name" text null, "bank_code" text null, "account_number" text null, "account_holder" text null, "iban" text null, "bic" text null, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "supplier_pkey" primary key ("id"));`,
		);
		this.addSql(

...
export class Migration20250702000000 extends Migration {
	...
	override async down(): Promise<void> {
		...
		this.addSql(
			`alter table if exists "product_supplier" drop column if exists "supplier_price_netto"`,
		);
		this.addSql(
			`alter table if exists "product_supplier" drop column if exists "supplier_price_brutto"`,
		);

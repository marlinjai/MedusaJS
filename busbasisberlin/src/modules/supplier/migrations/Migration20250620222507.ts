...
export class Migration20250620222507 extends Migration {
	override async up(): Promise<void> {
		...
		this.addSql(
			`alter table if exists "supplier" drop column if exists "salutation", drop column if exists "first_name", drop column if exists "last_name", drop column if exists "contact", drop column if exists "fax", drop column if exists "active", drop column if exists "language", drop column if exists "delivery_time";`,
		);

		this.addSql(
			`alter table if exists "supplier" alter column "company" type text using ("company"::text);`,
		);
		this.addSql(
			`alter table if exists "supplier" alter column "company" set not null;`,
		);
	}
	...
	override async down(): Promise<void> {
		...
		this.addSql(
			`alter table if exists "supplier" alter column "company" drop not null;`,
		);
	}
}

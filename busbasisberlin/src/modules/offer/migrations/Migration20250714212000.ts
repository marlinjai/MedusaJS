...
export class Migration20250714212000 extends Migration {
	async up(): Promise<void> {
		...
		this.addSql(
			`ALTER TABLE "offer_status_history" ADD COLUMN IF NOT EXISTS "notification_method" text NULL;`,
		);

		// Add the metadata column that's also in the model
		this.addSql(
			`ALTER TABLE "offer_status_history" ADD COLUMN IF NOT EXISTS "metadata" text NULL;`,
		);

		// Add the financial_impact column that's also in the model
		this.addSql(
			`ALTER TABLE "offer_status_history" ADD COLUMN IF NOT EXISTS "financial_impact" text NULL;`,
		);

		// Remove the incorrect communication_sent column (optional - comment out if you want to keep it)
		this.addSql(
			`ALTER TABLE "offer_status_history" DROP COLUMN IF EXISTS "communication_sent";`,
		);
	}
	...
	async down(): Promise<void> {
		...
		this.addSql(
			`ALTER TABLE "offer_status_history" DROP COLUMN IF EXISTS "notification_sent";`,
		);
		this.addSql(
			`ALTER TABLE "offer_status_history" DROP COLUMN IF EXISTS "notification_method";`,
		);
		this.addSql(
			`ALTER TABLE "offer_status_history" DROP COLUMN IF EXISTS "metadata";`,
		);
		this.addSql(
			`ALTER TABLE "offer_status_history" DROP COLUMN IF EXISTS "financial_impact";`,
		);
	}
}

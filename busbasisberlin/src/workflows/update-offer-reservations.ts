    ...
    for (const item of input.items_to_update) {
      try {
        ...
        if (offerItem?.reservation_id) {
          ...
          updatedCount++;
          logger.info(`[UPDATE-WORKFLOW] Updated reservation ${offerItem.reservation_id} to ${item.quantity} units`);
        } else {
          // Create new reservation if none exists (edge case)
          const inventoryItems = await inventoryService.listInventoryItems({ sku: item.sku });
          ...
          createdCount++;
          logger.info(`[UPDATE-WORKFLOW] Created new reservation for updated item ${item.title}`);
        }
      } catch (error) {
        logger.error(`[UPDATE-WORKFLOW] Failed to update reservation for item ${item.id}: ${error.message}`);
        throw error;
      }
    }
  ...
  async (
    ...
    for (const item of input.items_to_create) {
      try {
        ...
        createdCount++;
        logger.info(`[UPDATE-WORKFLOW] Created reservation for new item ${item.title}`);
      } catch (error) {
        logger.error(`[UPDATE-WORKFLOW] Failed to create reservation for new item ${item.id}: ${error.message}`);
        throw error;
      }
    }

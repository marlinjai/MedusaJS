...
const releaseOfferReservationsStep = createStep(
  ...
  async (input: { offer: any; productItems: any[]; reason?: string }, { container }) => {
    ...
    for (const item of input.productItems) {
      ...
      if (!item.reservation_id) {
        logger.info(`[RELEASE-WORKFLOW] Item ${item.title} has no reservation to release`);
        continue;
      }
      ...
      try {
        ...
        logger.error(`[RELEASE-WORKFLOW] Failed to release reservation ${item.reservation_id}: ${error.message}`);

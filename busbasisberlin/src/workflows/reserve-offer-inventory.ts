...
import { CreatedReservation, ReserveOfferInventoryInput } from '../modules/offer/types';
    ...
    for (const item of input.productItems) {
      try {
        ...
        if (inventoryItems.length === 0) {
          logger.warn(`[RESERVE-WORKFLOW] No inventory item found for SKU: ${item.sku}`);
          continue;
        }
        ...
        if (inventoryLevels.length === 0) {
          logger.warn(`[RESERVE-WORKFLOW] No inventory levels found for item: ${inventoryItem.id}`);
          continue;
        }
        ...
        logger.error(`[RESERVE-WORKFLOW] Failed to create reservation for item ${item.id}: ${error.message}`);

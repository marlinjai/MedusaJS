...
const validateFulfillmentStep = createStep(
  ...
  async (input: { offer_id: string }, { container }) => {
    ...
    logger.info(`[OFFER-FULFILLMENT] Validating fulfillment for offer ${input.offer_id}`);
    ...
    if (!['accepted', 'active'].includes(offer.status)) {
      throw new Error(`Offer ${input.offer_id} cannot be fulfilled - status is ${offer.status}`);
    }

    // Check if offer has reservations
    if (!offer.has_reservations) {
      logger.warn(`[OFFER-FULFILLMENT] Offer ${input.offer_id} has no reservations - proceeding anyway`);
    }
  ...
  async (compensationData, { container }) => {
    ...
    const logger = container.resolve(ContainerRegistrationKeys.LOGGER);
    logger.info(`[OFFER-FULFILLMENT] Validation step compensation - no action needed`);
  },
);
...
const reduceInventoryLevelsStep = createStep(
  ...
  async (input: { offer: any }, { container }) => {
    ...
    logger.info(`[OFFER-FULFILLMENT] Reducing inventory levels for offer ${input.offer.offer_number}`);
        ...
        try {
          ...
          for (const inventoryItem of inventoryItems) {
            ...
            for (const level of inventoryLevels) {
              ...
              const availableToReduce = Math.min(level.stocked_quantity - level.reserved_quantity, item.quantity);
              ...
              if (availableToReduce > 0) {
                ...
                logger.info(`[OFFER-FULFILLMENT] Reduced inventory for ${item.sku}: ${availableToReduce} units`);
              }
            }
          }
        } catch (error) {
          logger.error(`[OFFER-FULFILLMENT] Failed to reduce inventory for item ${item.sku}: ${error.message}`);
          throw error;
        }
      }
    }
    ...
    return new StepResponse(
      {
        ...
        total_quantity_reduced: reducedItems.reduce((sum, item) => sum + item.quantity_reduced, 0),
  ...
  async (compensationData, { container }) => {
    ...
    for (const item of compensationData.reduced_items) {
      try {
        ...
        logger.info(`[OFFER-FULFILLMENT] Restored ${item.quantity_reduced} units for ${item.sku}`);
      } catch (error) {
        logger.error(`[OFFER-FULFILLMENT] Failed to restore inventory for ${item.sku}: ${error.message}`);
      }
    }
  },
);
...
const releaseOfferReservationsStep = createStep(
  ...
  async (input: { offer_id: string }, { container }) => {
    ...
    logger.info(`[OFFER-FULFILLMENT] Releasing reservations for offer ${input.offer_id}`);
    ...
    try {
      ...
      logger.info(`[OFFER-FULFILLMENT] Reservations released: ${result.result.reservations_released} items`);
      ...
      logger.error(`[OFFER-FULFILLMENT] Failed to release reservations: ${error.message}`);
      throw error;
    }
  },
  async (compensationData, { container }) => {
    ...
    logger.info(`[OFFER-FULFILLMENT] Compensating: Re-creating released reservations`);
    // Note: Re-creating reservations after fulfillment is complex
    // In practice, you might want to log this for manual intervention
    logger.warn(`[OFFER-FULFILLMENT] Reservation re-creation not implemented - manual intervention may be required`);
  },
);
...
const updateOfferStatusStep = createStep(
  ...
  async (input: { offer_id: string }, { container }) => {
    ...
    logger.info(`[OFFER-FULFILLMENT] Updating offer ${input.offer_id} status to completed`);
  ...
  async (compensationData, { container }) => {
    ...
    try {
      ...
      logger.error(`[OFFER-FULFILLMENT] Failed to revert status: ${error.message}`);
    }
  },
);
...
export const fulfillOfferReservationsWorkflow = createWorkflow(
  ...
  (input: { offer_id: string; user_id?: string }) => {
    ...
    return new WorkflowResponse({
      ...
      reservations_released_count: releaseReservations.reservations_released_count,
    });
  },
);

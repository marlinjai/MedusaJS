    ...
    try {
      ...
      let existingSupplier = existingSuppliers.find(
        s =>
          ...
          (safe(row, 'Lieferantennummer') && s.supplier_number === safe(row, 'Lieferantennummer')),
      );
      ...
      if (existingSupplier) {
        ...
        const updateData = {
          ...
          company_addition: existingSupplier.company_addition || safe(row, 'Firmenzusatz'),
          supplier_number: existingSupplier.supplier_number || safe(row, 'Lieferantennummer'),
          customer_number: existingSupplier.customer_number || safe(row, 'Eigene Kd-Nr'),
          internal_key: existingSupplier.internal_key || safe(row, 'Interner Schlüssel'),
          vat_id: existingSupplier.vat_id || safe(row, 'UstID'),
          status: existingSupplier.status || safe(row, 'Status') || 'active',
          is_active: existingSupplier.is_active !== undefined ? existingSupplier.is_active : safe(row, 'Aktiv') === 'Y',
          language: existingSupplier.language || safe(row, 'Sprache') || 'Deutsch',
          lead_time:
            existingSupplier.lead_time || (safe(row, 'Lieferzeit') ? parseInt(safe(row, 'Lieferzeit')!) : null),
          ...
          account_number: existingSupplier.account_number || safe(row, 'KontoNr'),
          account_holder: existingSupplier.account_holder || safe(row, 'Inhaber'),
        ...
        const supplierData = {
          ...
          lead_time: safe(row, 'Lieferzeit') ? parseInt(safe(row, 'Lieferzeit')!) : null,
      ...
      if (hasContactData) {
        const existingContacts = await supplierService.listContacts({ supplier_id: supplier.id });

        // Check if we already have a main contact with the same name
        const existingMainContact = existingContacts.find(
          c => c.is_main_contact && c.first_name === safe(row, 'Vorname') && c.last_name === safe(row, 'Nachname'),
        );
        ...
        if (existingMainContact) {
          ...
          const createdContacts = await supplierService.createContacts([contactData]);
          contact = createdContacts[0];
          console.log(`    ✓ Created contact: ${contact.first_name} ${contact.last_name}`);
        } else {
          console.log(`    ⚠ Skipping contact creation - contacts already exist`);
        ...
        if (contact) {
          ...
          for (const phone of phoneNumbers) {
            const phoneExists = existingPhones.some(p => p.number === phone.number);
            if (!phoneExists) {
              ...
              console.log(`      ✓ Added phone: ${phone.number} (${phone.label})`);
            } else {
      ...
      const hasAddressData = safe(row, 'Strasse') || safe(row, 'PLZ') || safe(row, 'Ort');

      if (hasAddressData) {
        const existingAddresses = await supplierService.listSupplierAddresses({ supplier_id: supplier.id });

        // Check if we already have an address with the same street/city
        const addressExists = existingAddresses.some(
          a => a.street === safe(row, 'Strasse') && a.city === safe(row, 'Ort') && a.postal_code === safe(row, 'PLZ'),
        );

        if (!addressExists) {
          const addressData = {
            supplier_id: supplier.id,
            label: existingAddresses.length === 0 ? 'Hauptsitz' : `Adresse ${existingAddresses.length + 1}`,
          ...
          console.log(`    ✓ Created address: ${addressData.street}, ${addressData.postal_code} ${addressData.city}`);
        } else {

...
export default async function importProducts({ container }: ExecArgs) {
  ...
  logger.info('🚀 Starting comprehensive product import with hierarchical categories...');
  ...
  try {
    ...
    if (salesChannels.length > 0) {
      ...
      logger.error('❌ No sales channels found! Products will not be visible in storefront.');
      logger.info('💡 Create a sales channel in the admin panel to enable product visibility.');
      return;
    }
  } catch (error) {
  ...
  try {
    ...
    if (stockLocations.length > 0) {
      ...
      logger.info(`📍 Using stock location for inventory: ${stockLocation.name} (${stockLocation.id})`);
    } else {
      logger.warn('⚠️  No stock locations found. Products will not be linked to inventory locations.');
      logger.info('💡 Create a stock location in the admin panel to enable inventory tracking.');
    }
  } catch (error) {
  ...
  const imageCsvPath = path.resolve(__dirname, '..', '..', '..', 'data', 'JTL-Wawi-Bildexport-02052025.csv');
  const imageMappingPath = path.resolve(__dirname, '..', '..', '..', 'data', 'image-url-mapping.json');
  ...
  try {
    ...
    logger.info(`📋 Loaded image mapping with ${Object.keys(imageMapping).length} entries`);
  } catch (error) {
  ...
  try {
    ...
    logger.info(`📸 Created image mapping for ${Object.keys(productImageMapping).length} products`);
  } catch (error) {
  ...
  try {
    ...
    logger.info(`📂 Built category tree with ${flatCategories.length} categories across 4 levels`);
    ...
    const categoriesByLevel = [1, 2, 3, 4].map(level => flatCategories.filter(cat => cat.level === level));

    for (let level = 1; level <= 4; level++) {
      const levelCategories = categoriesByLevel[level - 1];
      if (levelCategories.length === 0) continue;

      logger.info(`📂 Creating Level ${level} categories (${levelCategories.length} categories)...`);
      ...
      if (newCategories.length === 0) {
        logger.info(`📂 All Level ${level} categories already exist, skipping creation`);
        // Map remaining existing categories
        levelCategories.forEach(cat => {
          if (!cat.id) {
            const existingByHandle = existingHandleMap.get(cat.handle.toLowerCase());
      ...
      for (let i = 0; i < newCategories.length; i += CATEGORY_BATCH_SIZE) {
        ...
        const categoryData = batch.map(cat => ({
          ...
          handle: cat.handle,
          is_active: true,
          parent_category_id: cat.parent && cat.parent.level > 0 ? categoryIdMap.get(cat.parent.handle) : undefined,
        }));
        ...
        try {
          ...
          logger.info(`✅ Created batch of ${createdCategories.length} Level ${level} categories`);
          ...
          for (const cat of batch) {
            try {
                  ...
                  product_categories: [
                    {
                      ...
                      is_active: true,
                      parent_category_id:
                        cat.parent && cat.parent.level > 0 ? categoryIdMap.get(cat.parent.handle) : undefined,
                    },
                  ],
                },
              });
              ...
              cat.id = created.id;
              logger.info(`✅ Created individual category: ${cat.name} (${cat.handle})`);
            } catch (individualError) {
              logger.warn(`⚠️  Skipped problematic category: ${cat.name} (${cat.handle}) - ${individualError.message}`);
              // Check if it exists now and map it
              const refreshedCategories = await productService.listProductCategories();
              const found = refreshedCategories.find(
                existing => existing.handle === cat.handle || existing.name === cat.name,
              );
              if (found) {
                categoryIdMap.set(cat.handle, found.id);
                cat.id = found.id;
                logger.info(`✅ Mapped to existing category: ${cat.name} (${found.handle})`);
              }
            }
          }
        }
    ...
    logger.info(`✅ Category hierarchy creation complete! Created/mapped ${categoryIdMap.size} categories`);
    ...
    const duplicateSKUs = Array.from(skuCounts.entries()).filter(([sku, count]) => count > 1);
    ...
    if (duplicateSKUs.length > 0) {
      ...
      duplicateSKUs.slice(0, 5).forEach(([sku, count]) => {
        ...
        logger.warn(`⚠️  SKU "${sku}" appears ${count} times with suppliers: ${suppliers.join(', ')}`);
      });
    ...
    articles.forEach(article => {
      const sku = safe(article, 'Artikelnummer');
      if (!sku) {
        logger.warn(`⚠️  Skipping article without SKU: ${JSON.stringify(article).substring(0, 100)}...`);
        return;
      }
    ...
    logger.info(`📦 Grouped ${articles.length} CSV rows into ${productGroups.size} unique products`);

    // Show grouping stats
    const multiSupplierProducts = Array.from(productGroups.entries()).filter(([sku, rows]) => rows.length > 1);
    logger.info(`🔀 ${multiSupplierProducts.length} products have multiple suppliers`);
      ...
      for (const sku of batch) {
        try {
          ...
          if (!productRows || productRows.length === 0) {
            logger.warn(`⚠️  Skipping product with missing required fields: ${sku}`);
            skippedCount++;
            continue;
          }
          ...
          try {
            const [existingProducts] = await productService.listProducts({ sku: sku });
            if (existingProducts && existingProducts.length && existingProducts.length > 0) {
              existingProduct = existingProducts[0];
              logger.info(`🔄 Found existing product: ${sku} - will update with missing data`);
            }
          } catch (error) {
          ...
          const weightInKg = parseNumber(safe(baseArticle, 'Artikelgewicht in KG') || '0');
          ...
          // Parse stock
          const stock = parseInt(safe(baseArticle, 'Lagerbestand Gesamt') || '0') || 0;
          ...
          if (categoryLevels.length > 0) {
            ...
            for (let depth = categoryLevels.length; depth > 0; depth--) {
              ...
              const handle = createHandle(categoryLevels.slice(0, depth).join('-'));
          ...
          if (supplierName) {
            logger.info(`🏢 Product ${sku}: Supplier: "${supplierName}", Manufacturer: "${manufacturerName || 'N/A'}"`);
          }

          // Before creating each product (inside the product import loop, before productToCreate is used)
          console.log(`Product: ${sku}, categoryPath: "${categoryPath}", categoryId: ${categoryId}`);
          ...
          if (successCount < 5) {
            ...
            if (categoryLevels.length > 0) {
              for (let depth = categoryLevels.length; depth > 0; depth--) {
                ...
                console.log(`   Depth ${depth}: Path="${path}", Handle="${handle}", Found=${hasHandle}`);
              }
            }
          }
          ...
          const productToCreate = {
            ...
            is_giftcard: false,
            discountable: true,
            status: safe(baseArticle, 'Aktiv') === 'Y' ? ProductStatus.PUBLISHED : ProductStatus.DRAFT,
            ...
            variants: [
              {
                ...
                metadata: {
                  ...
                  lead_time:
                    parseInt(safe(baseArticle, 'Beschaffungszeit manuell ermitteln (Tage)') || '0') || undefined,
                  min_quantity: parseInt(safe(baseArticle, 'Mindestabnahme') || '1') || 1,
                  ...
                  supplier_article_name: safe(baseArticle, 'Lieferanten Artikelname'),
            ...
            metadata: {
              ...
              purchase_price: supplierPrice,
              min_stock: parseInt(safe(baseArticle, 'Mindestlagerbestand') || '0') || 0,
              original_sku: sku,
              internal_key: safe(baseArticle, 'Interner Schlüssel'),
              supplier_article_name: safe(baseArticle, 'Lieferanten Artikelname'),
              notes: safe(baseArticle, 'Anmerkung'),
          ...
          if (existingProduct) {
            ...
            try {
              await productService.updateProducts(existingProduct.id, {
                ...
                description: existingProduct.description || productToCreate.description,
                ...
                metadata: {
                  ...
                  supplier_name: existingProduct.metadata?.supplier_name || productToCreate.metadata?.supplier_name,
                  category_path: existingProduct.metadata?.category_path || productToCreate.metadata?.category_path,
                  category_level_1:
                    existingProduct.metadata?.category_level_1 || productToCreate.metadata?.category_level_1,
                  category_level_2:
                    existingProduct.metadata?.category_level_2 || productToCreate.metadata?.category_level_2,
                  category_level_3:
                    existingProduct.metadata?.category_level_3 || productToCreate.metadata?.category_level_3,
                  category_level_4:
                    existingProduct.metadata?.category_level_4 || productToCreate.metadata?.category_level_4,
                  internal_key: existingProduct.metadata?.internal_key || productToCreate.metadata?.internal_key,
                  supplier_article_name:
                    existingProduct.metadata?.supplier_article_name || productToCreate.metadata?.supplier_article_name,
                  notes: existingProduct.metadata?.notes || productToCreate.metadata?.notes,
                  last_modified: existingProduct.metadata?.last_modified || productToCreate.metadata?.last_modified,
                },
              });
              ...
              for (const article of productRows) {
                ...
                if (rowSupplierName && supplierMap.has(rowSupplierName.toLowerCase())) {
                  ...
                  try {
                    ...
                    const supplierProductName = safe(article, 'Lieferanten Artikelname');

                    if (supplierProductName && supplierProductName !== safe(article, 'Artikelname')) {
                      // Use supplier's product name if different from main product name
                      variantName = supplierProductName;
                    } else if (rowManufacturerName && rowManufacturerName !== 'N/A') {
                    ...
                    const supplierNettoPrice = parsePrice(safe(article, 'EK Netto') || '0', true); // Supplier netto price (cents)
                    const supplierBruttoPrice = parsePrice(safe(article, 'EK Brutto') || '0', true); // Supplier brutto price (cents)
                    const supplierVatRate = parseFloat(safe(article, 'USt. in %') || '0') || null; // Supplier VAT rate

                    await supplierService.linkProductToSupplier(existingProduct.id, supplier.id, {
                      variant_name: variantName,
                      variant_description: safe(article, 'Lieferanten Artikelname'),
                      supplier_sku: safe(article, 'Lieferanten-Art.Nr.'),
                      supplier_product_name: safe(article, 'Lieferanten Artikelname'),
                      ...
                      supplier_lead_time: parseInt(safe(article, 'Lieferanten Lieferzeit') || '0') || null,
                      supplier_delivery_time: parseInt(safe(article, 'Lieferfrist') || '0') || null,
                      supplier_min_order_qty: parseInt(safe(article, 'Mindestabnahme Lieferant') || '1') || 1,
                      supplier_order_interval: parseInt(safe(article, 'Lieferant Abnahmeintervall') || '0') || null,
                      supplier_stock: parseInt(safe(article, 'Lieferantenbestand') || '0') || null,
                      supplier_comment: safe(article, 'Kommentar'),
                      supplier_merge_stock: safe(article, 'Lagerbestand zusammenführen') === 'Y',
                      is_dropshipping: safe(article, 'IstDropshippingartikel') === 'Y',
                      use_supplier_lead_time: safe(article, 'Lieferzeit vom Lieferanten beziehen') === 'Y',
                      is_primary: safe(article, 'Ist Standardlieferant') === 'Y',
                      is_active: true,
                      sort_order: 0,
                    ...
                    logger.warn(`⚠️  Failed to create supplier relationship for ${sku}: ${supplierError.message}`);
                  }
                }
              }
              ...
              if (stockLocation && stock > 0) {
                try {
                  ...
                  const relevantInventoryItems = productInventoryItems.filter((item: any) =>
                    productVariantSkus.includes(item.sku),
                  );

                  if (relevantInventoryItems.length > 0) {
                    // Create inventory levels for each inventory item
                    const inventoryLevels = relevantInventoryItems.map((item: any) => ({
                    ...
                    logger.info(`📦 Created inventory level for ${sku}: ${stock} units at ${stockLocation.name}`);
                  } else {
                    logger.warn(`⚠️  No inventory items found for product ${sku}`);
                  }
                } catch (inventoryError) {
                  logger.warn(`⚠️  Failed to create inventory level for ${sku}: ${inventoryError.message}`);
                }
              }
            } catch (updateError) {
              logger.error(`❌ Failed to update existing product ${sku}: ${updateError.message}`);
              errorCount++;
              continue;
            }
          } else {
            ...
            for (const article of productRows) {
              ...
              if (rowSupplierName && supplierMap.has(rowSupplierName.toLowerCase())) {
                ...
                try {
                  ...
                  await supplierService.linkProductToSupplier(createdProduct.id, supplier.id, {
                    variant_name: variantName,
                    variant_description: safe(article, 'Lieferanten Artikelname'),
                    supplier_sku: safe(article, 'Lieferanten-Art.Nr.'),
                    supplier_product_name: safe(article, 'Lieferanten Artikelname'),
    ...
    if (successCount > 0) {
      logger.info(`🎯 Successfully imported ${successCount} products with proper hierarchical categories!`);
      logger.info(`🔗 Check the Medusa admin at /app/categories to see the category hierarchy`);
    }
  } catch (error) {

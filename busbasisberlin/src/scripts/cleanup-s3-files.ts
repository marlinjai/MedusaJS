...
export default async function cleanupS3Files({ container }: ExecArgs) {
	...
	try {
		...
		if (
			!supabaseConfig.credentials.accessKeyId ||
			!supabaseConfig.credentials.secretAccessKey
		) {
		...
		logger.info(
			`Connecting to Supabase Storage with endpoint: ${supabaseConfig.endpoint}`,
		);
		...
		logger.info(
			`Found ${imageUrls.size} image URLs and ${imageKeys.size} distinct file keys in use by products`,
		);
		...
		logger.info(
			`Found ${allS3Objects.length} total files in Supabase Storage bucket`,
		);
		...
		const orphanedFiles = allS3Objects.filter(obj => {
			...
			const isInImageUrl = Array.from(imageUrls).some(
				url => url.includes(key) || url.includes(fileName),
			);
		...
		logger.info(
			`Found ${orphanedFiles.length} orphaned files in Supabase Storage`,
		);
		...
		logger.info(
			`Supabase Storage cleanup completed. Deleted ${deletedCount} orphaned files.`,
		);
	} catch (error) {

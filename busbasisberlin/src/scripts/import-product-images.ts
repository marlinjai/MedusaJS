...
export default async function importProductImages({ container }: ExecArgs) {
	...
	for (let i = 0; i < filesToUpload.length; i += BATCH_SIZE) {
			...
			for (let attempt = 1; attempt <= RETRY_ATTEMPTS; attempt++) {
				try {
					...
					if (attempt === RETRY_ATTEMPTS) {
						logger.error(
							`❌ Failed to upload ${file} after ${RETRY_ATTEMPTS} attempts: ${error.message}`,
						);
						errorCount++;
					} else {
						logger.warn(
							`⚠️  Attempt ${attempt}/${RETRY_ATTEMPTS} failed for ${file}: ${error.message}`,
						);
						// Wait before retry
		...
		try {
			...
			logger.info(
				`💾 Saved mapping file with ${Object.keys(newMapping).length} entries`,
			);
		} catch (error) {
	...
	logger.info(
		`   📁 Total images in mapping: ${Object.keys(newMapping).length}`,
	);

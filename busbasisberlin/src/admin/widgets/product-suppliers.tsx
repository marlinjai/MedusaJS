...
const ProductSuppliersWidget = ({ data: product }: { data: { id: string } }) => {
  ...
  const { data: groupedSuppliers = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(`/admin/products/${product.id}/suppliers/grouped`, {
        credentials: 'include',
      });
  ...
  const addSupplierMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['product-suppliers-grouped', product.id] });
  ...
  const updateRelationshipMutation = useMutation({
    mutationFn: async ({ relationshipId, data }: { relationshipId: string; data: any }) => {
      const res = await fetch(`/admin/products/${product.id}/suppliers/relationships/${relationshipId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
  ...
  const startEditing = (relationshipId: string, field: string, currentValue: any) => {
    setEditingCell({ relationshipId, field, value: currentValue });
    setTempValues({ ...tempValues, [`${relationshipId}-${field}`]: currentValue });
  };
  ...
  const saveEdit = () => {
    ...
    if (['supplier_price', 'supplier_price_netto', 'supplier_price_brutto'].includes(editingCell.field) && newValue) {
      processedValue = Math.round(parseFloat(newValue) * 100); // Convert to cents
    } else if (['supplier_lead_time', 'supplier_stock', 'sort_order', 'supplier_vat_rate'].includes(editingCell.field)) {
  ...
  const handleDragEnd = (result: any) => {
    ...
    if (source.droppableId === destination.droppableId) {
      ...
      const supplierGroup = groupedSuppliers.find(g => g.supplier.id === supplierId);
  ...
  const renderEditableCell = (relationship: ProductSupplier, field: string, currentValue: any) => {
    const isEditing = editingCell?.relationshipId === relationship.id && editingCell?.field === field;
      ...
      if (field === 'notes' || field === 'variant_description') {
        return (
          <Textarea
            ...
            placeholder={field === 'variant_description' ? 'Enter description...' : 'Enter notes...'}
          />
        );
      } else {
        const inputType = ['supplier_price', 'supplier_price_netto', 'supplier_price_brutto', 'supplier_vat_rate'].includes(field) ? 'number' :
                          ['supplier_lead_time', 'supplier_stock', 'sort_order'].includes(field) ? 'number' : 'text';
        const step = ['supplier_price', 'supplier_price_netto', 'supplier_price_brutto'].includes(field) ? '0.01' :
                    field === 'supplier_vat_rate' ? '0.1' : undefined;
        ...
        return (
          <Input
            ...
            placeholder={field === 'supplier_vat_rate' ? '19.0' : field === 'supplier_price' ? '0.00' : 'Enter value...'}
          />
        );
      }
    }
    ...
    if (field === 'supplier_price' || field === 'supplier_price_netto' || field === 'supplier_price_brutto') {
    ...
    return (
      ...
      >
        <Text size="small" className="truncate w-full" title={displayValue || '—'}>
          {displayValue || '—'}
        </Text>
      </div>
    );
      ...
      <div className="space-y-2">
        {groupedSuppliers.length === 0 ? (
          <div className="text-center py-8 border-2 border-dashed border-ui-border-base rounded-lg">
            <Text className="text-ui-fg-subtle">No suppliers for this product</Text>
            <Text className="text-ui-fg-muted text-sm mt-1">Add a supplier to get started</Text>
          </div>
        ) : (
          <DragDropContext onDragEnd={handleDragEnd}>
            {groupedSuppliers.map((group) => (
              <div key={group.supplier.id} className="border border-ui-border-base rounded-lg overflow-hidden">
                {/* Supplier Header */}
                <div
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <Badge size="small" className="bg-ui-bg-subtle-pressed text-ui-fg-subtle">
                      {group.totalRelationships} {group.totalRelationships === 1 ? 'entry' : 'entries'}
                    </Badge>
                ...
                {expandedSuppliers.has(group.supplier.id) && group.relationships.length > 0 && (
                  <div className="bg-ui-bg-base overflow-x-auto">
                    <table className="w-full min-w-[1200px] table-fixed">
                      <thead>
                        <tr className="border-b border-ui-border-base bg-ui-bg-subtle">
                          <th className="w-12 p-2 text-xs font-medium text-ui-fg-subtle text-center">Drag</th>
                          <th className="w-32 p-2 text-xs font-medium text-ui-fg-subtle text-left">Variant</th>
                          <th className="w-24 p-2 text-xs font-medium text-ui-fg-subtle text-left">Description</th>
                          <th className="w-20 p-2 text-xs font-medium text-ui-fg-subtle text-left">Netto €</th>
                          <th className="w-20 p-2 text-xs font-medium text-ui-fg-subtle text-left">Brutto €</th>
                          <th className="w-16 p-2 text-xs font-medium text-ui-fg-subtle text-left">VAT %</th>
                          <th className="w-32 p-2 text-xs font-medium text-ui-fg-subtle text-left">SKU</th>
                          <th className="w-20 p-2 text-xs font-medium text-ui-fg-subtle text-left">Lead Time</th>
                          <th className="w-16 p-2 text-xs font-medium text-ui-fg-subtle text-left">Stock</th>
                          <th className="w-16 p-2 text-xs font-medium text-ui-fg-subtle text-center">Primary</th>
                          <th className="w-16 p-2 text-xs font-medium text-ui-fg-subtle text-center">Fav</th>
                          <th className="w-24 p-2 text-xs font-medium text-ui-fg-subtle text-left">Notes</th>
                          <th className="w-16 p-2 text-xs font-medium text-ui-fg-subtle text-center">Actions</th>
                        </tr>
                      </thead>
                                ...
                                {(provided: any, snapshot: any) => (
                                  <tr
                                    ...
                                    className={`border-b border-ui-border-base hover:bg-ui-bg-subtle ${
                                      snapshot.isDragging ? 'shadow-lg bg-ui-bg-component' : ''
                                    }`}
                                  >
                                      <td className="p-2 text-center">
                                        <div {...provided.dragHandleProps} className="cursor-grab p-1 hover:bg-ui-bg-subtle rounded inline-block">
                                      ...
                                      <td className="p-2">
                                        ...
                                        <div className="p-1 h-[32px] flex items-center w-full">
                                          <Text size="small" className="truncate w-full" title={productData?.title || '—'}>
                                            {productData?.title || '—'}
                                          </Text>
                                        </div>
                                      </td>
                                      <td className="p-2">
                                        {renderEditableCell(relationship, 'variant_description', relationship.variant_description)}
                                      </td>
                                      <td className="p-2">
                                        {renderEditableCell(relationship, 'supplier_price_netto', relationship.supplier_price_netto)}
                                      </td>
                                      <td className="p-2">
                                        {renderEditableCell(relationship, 'supplier_price_brutto', relationship.supplier_price_brutto)}
                                      </td>
                                      <td className="p-2">
                                        {renderEditableCell(relationship, 'supplier_vat_rate', relationship.supplier_vat_rate)}
                                      </td>
                                      <td className="p-2">
                                        {renderEditableCell(relationship, 'supplier_sku', relationship.supplier_sku)}
                                      </td>
                                      <td className="p-2">
                                        {renderEditableCell(relationship, 'supplier_lead_time', relationship.supplier_lead_time)}
                                      </td>
                                      <td className="p-2">
                                        {renderEditableCell(relationship, 'supplier_stock', relationship.supplier_stock)}
                                      </td>
                                      <td className="p-2 text-center">
                                        {renderEditableCell(relationship, 'is_primary', relationship.is_primary)}
                                      </td>
                                      <td className="p-2 text-center">
                                        {renderEditableCell(relationship, 'is_favorite', relationship.is_favorite)}
                                      </td>
                                      <td className="p-2">
                                        {renderEditableCell(relationship, 'notes', relationship.notes)}
                                      </td>
                                      <td className="p-2 text-center">
                                        <DropdownMenu>
                                          ...
                                          <DropdownMenu.Content>
                                            <DropdownMenu.Item
                                              onClick={() => {
                                                // Copy relationship
                                                const copyData = { ...relationship };
                                                const { id: _, ...dataWithoutId } = copyData;
                                                const updatedData = {
                                                  ...dataWithoutId,
                                                  variant_name: `${copyData.variant_name || 'Copy'} - Copy`
                                                };

                                                fetch(`/admin/products/${product.id}/suppliers`, {
                                                  method: 'POST',
                                                  headers: { 'Content-Type': 'application/json' },
                                                  credentials: 'include',
                                                  ...
                                                  queryClient.invalidateQueries({ queryKey: ['product-suppliers-grouped', product.id] });
                                                  toast.success('Relationship copied');
                                                });
                                              }}
                                            >

...
const SearchableDropdown = ({
  ...
  const getSubtitle = (item: SearchableItem): string => {
    switch (itemType) {
      case 'customer':
        return [item.email, item.phone, item.customer_number].filter(Boolean).join(' • ');
      case 'product':
        // Convert cents to euros for display
        const productPrice = item.unit_price ? `${(item.unit_price / 100).toFixed(2)} €` : null;
        return [item.sku, item.category, productPrice].filter(Boolean).join(' • ');
      case 'service':
        // Convert cents to euros for display
        const servicePrice = item.base_price ? `${(item.base_price / 100).toFixed(2)} €` : null;
        return [item.category, item.service_type, servicePrice].filter(Boolean).join(' • ');
      default:
        return '';
    }
  };

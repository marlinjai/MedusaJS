...
const SuppliersPage = () => {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch('/admin/suppliers?withDetails=true', { credentials: 'include' });
      ...
      if (supplier.contacts) {
        const contactMatch = supplier.contacts.some(
          (contact: SupplierWithDetails['contacts'][0]) =>
            ...
            contact.emails?.some((email: { email: string; label?: string }) => email.email?.toLowerCase().includes(searchLower)) ||
            contact.phones?.some((phone: { number: string; label?: string }) => phone.number?.includes(searchTerm)),
        );
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
          <div className="bg-ui-bg-subtle rounded-lg p-4">
            <div className="flex items-center">
              <div className="ml-3">
                <div className="text-xs font-medium text-ui-fg-subtle">Lieferanten gesamt</div>
          ...
          <div className="bg-ui-bg-subtle rounded-lg p-4">
            <div className="flex items-center">
              <div className="ml-3">
                <div className="text-xs font-medium text-ui-fg-subtle">Aktive</div>
          ...
          <div className="bg-ui-bg-subtle rounded-lg p-4">
            <div className="flex items-center">
              <div className="ml-3">
                <div className="text-xs font-medium text-ui-fg-subtle">Mit Kontakten</div>
                <div className="text-lg font-semibold">{stats.withContacts}</div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-ui-bg-subtle rounded-lg p-4">
            <div className="flex items-center">
              <div className="ml-3">
                <div className="text-xs font-medium text-ui-fg-subtle">Mit Adressen</div>
                <div className="text-lg font-semibold">{stats.withAddresses}</div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-ui-bg-subtle rounded-lg p-4">
            <div className="flex items-center">
              <div className="ml-3">
                <div className="text-xs font-medium text-ui-fg-subtle">Mit USt-ID</div>
          ...
          <div className="bg-ui-bg-subtle rounded-lg p-4">
            <div className="flex items-center">
              <div className="ml-3">
                <div className="text-xs font-medium text-ui-fg-subtle">Mit Bankdaten</div>
                <div className="text-lg font-semibold">{stats.withBankInfo}</div>
              </div>
            </div>
          </div>
        </div>

...
export async function GET(req: MedusaRequest<OfferParams>, res: MedusaResponse): Promise<void> {
  ...
  try {
    ...
    const now = new Date();
    const hoursSinceGenerated = (now.getTime() - lastModified.getTime()) / (1000 * 60 * 60);
    ...
    try {
      ...
      logger.warn(`[PDF-CACHE] Could not get offer number for ${offerId}, using fallback`);
    }
    ...
    try {
      ...
      logger.error(`[PDF-CACHE] Error serving PDF for offer ${offerId}:`, error);
      res.status(500).json({

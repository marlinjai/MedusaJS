...
export async function POST(
	req: MedusaRequest<TransitionStatusRequest>,
	res: MedusaResponse,
) {
	try {
		...
		if (
			['reservations_created', 'reservations_released'].includes(
				result.result.inventory_action,
			)
		) {
			try {
					...
					if (variantIds.length > 0) {
						...
						inventoryStatus = {
							...
							items_available: productItems.filter(item => {
								const availableQty =
									availability[item.variant_id!]?.availability || 0;
				...
				console.warn(
					'Failed to auto-refresh inventory after status transition:',

...
async function triggerInventoryRefresh(
	...
	try {
		logger.info(
			`[INVENTORY-REFRESH] Auto-triggering inventory check for offer ${offerId}`,
		);

		// Call the inventory check endpoint internally
		const inventoryResponse = await fetch(
			`http://localhost:9000/admin/offers/${offerId}/check-inventory`,
			{
				method: 'GET',
				headers: {
		...
		if (inventoryResponse.ok) {
			...
			logger.info(
				`[INVENTORY-REFRESH] Inventory refresh completed for offer ${offerId}: ${inventoryData.status}`,
			);
		} else {
			logger.warn(
				`[INVENTORY-REFRESH] Inventory refresh failed for offer ${offerId}: ${inventoryResponse.status}`,
			);
		}
	} catch (error) {
		logger.error(
			`[INVENTORY-REFRESH] Error refreshing inventory for offer ${offerId}:`,
			error,
		);
...
export async function PUT(
	req: MedusaRequest<UpdateOfferRequest>,
	res: MedusaResponse,
): Promise<void> {
	const offerService: OfferService = req.scope.resolve(OFFER_MODULE);
	const logger = req.scope.resolve(ContainerRegistrationKeys.LOGGER);
	...
	try {
		...
		if (updateData.valid_until !== undefined) {
			updateFields.valid_until = updateData.valid_until
		...
		if (updateData.items !== undefined) {
			...
			logger.info(
				`[RESERVATION-DEBUG] Current items: ${currentItems.length}, New items: ${newItems.length}`,
			);
			...
			if (
				hasProductItemChanges &&
				['active', 'accepted'].includes(existingOffer.status)
			) {
				try {
					...
					const workflowInput = {
						...
						items_to_update: itemsToUpdate
							.filter(
								item =>
									item.item_type === 'product' && item.variant_id && item.sku,
							)
							.map(item => ({
								id: item.id!,
					...
					if (!reservationUpdateResponse.ok) {
						...
						throw new Error(
							`Reservation update failed: ${errorData.error || 'Unknown error'}`,
						);
					}

					const reservationResult = await reservationUpdateResponse.json();
					logger.info(
						`[RESERVATION-UPDATE] Pre-deletion API call completed: ${reservationResult.message}`,
					);
					...
					logger.error(
						`Failed to update reservations for offer ${existingOffer.offer_number}:`,
						reservationError,
					);
				...
				if (itemData.id) {
					await offerService.updateOfferItems({
						...
						total_price:
							itemData.unit_price *
							itemData.quantity *
							(1 - (itemData.discount_percentage || 0) / 100),
					});
				}
			}
			...
			if (
				...
				try {
					...
					logger.info(
						`[RESERVATION-UPDATE] Processing item updates: ${JSON.stringify(workflowInput)}`,
					);
					...
					if (!reservationUpdateResponse.ok) {
						const errorData = await reservationUpdateResponse.json();
						throw new Error(
							`Item update reservation failed: ${errorData.error || 'Unknown error'}`,
						);
					}

					const reservationResult = await reservationUpdateResponse.json();
					logger.info(
						`[RESERVATION-UPDATE] Item update reservations completed: ${reservationResult.message}`,
					);
			...
			if (
				...
				try {
					...
					logger.info(
						`[RESERVATION-UPDATE] Processing new item reservations: ${JSON.stringify(workflowInput)}`,
					);
					...
					if (!reservationUpdateResponse.ok) {
						const errorData = await reservationUpdateResponse.json();
						throw new Error(
							`New item reservation failed: ${errorData.error || 'Unknown error'}`,
						);
					}

					const reservationResult = await reservationUpdateResponse.json();
					logger.info(
						`[RESERVATION-UPDATE] New item reservations completed: ${reservationResult.message}`,
					);

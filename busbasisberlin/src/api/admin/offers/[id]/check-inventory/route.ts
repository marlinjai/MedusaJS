  ...
  try {
    ...
    if (variantIds.length > 0) {
      try {
        ...
        logger.info(`[INVENTORY-CHECK] Retrieved inventory for ${Object.keys(inventoryMap).length} variants`);
      } catch (error) {
        logger.error(`[INVENTORY-CHECK] Error getting variant availability: ${error.message}`);

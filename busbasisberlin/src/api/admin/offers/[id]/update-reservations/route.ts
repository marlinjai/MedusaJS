...
export async function POST(
	req: MedusaRequest<UpdateReservationsRequest>,
	res: MedusaResponse,
) {
	try {
		const {
		...
		if (!hasDeletes && !hasUpdates && !hasCreates) {
			return res.status(400).json({
				success: false,
				error:
					'At least one of items_to_delete, items_to_update, or items_to_create must have elements',
			});
		}
		...
		if (items_to_update && !Array.isArray(items_to_update)) {
			return res.status(400).json({
				success: false,
				error:
					'items_to_update must be an array of objects with id, variant_id, sku, quantity, and title',
			});
		}
		...
		if (items_to_create && !Array.isArray(items_to_create)) {
			return res.status(400).json({
				success: false,
				error:
					'items_to_create must be an array of objects with id, variant_id, sku, quantity, title, and item_type',
			});
		}
			...
			for (const item of items_to_update) {
				if (
					...
					return res.status(400).json({
						success: false,
						error:
							'Each item in items_to_update must have id, variant_id, sku, quantity (number), and title',
					});
				}
			}
		}
		...
		const result = await updateOfferInventoryReservationsWorkflow(
			req.scope,
		).run({
			input: {
				offer_id,

...
export const GET = async (req: MedusaRequest, res: MedusaResponse) => {
	...
	try {
		...
		const relationships =
			await supplierService.getSuppliersForProduct(productId);

		// For each relationship, get the supplier details
		const suppliersWithData = await Promise.all(
			relationships.map(async relationship => {
				const supplier = await supplierService.retrieveSupplier(
...
export const POST = async (req: MedusaRequest, res: MedusaResponse) => {
	...
	try {
		...
		const relationship = await supplierService.linkProductToSupplier(
			productId,
			supplier_id,
			relationshipData,
		);

...
export async function POST(req: MedusaRequest, res: MedusaResponse) {
	try {
		const { id } = req.params;
		const { core_customer_id, linking_method } =
			req.body as LinkCustomerRequest;
		...
		const linkedCustomer = await manualCustomerService.linkToCustomer(
			id,
			core_customer_id,
			linking_method,
		);
		...
		res.status(500).json({
			message:
				error instanceof Error ? error.message : 'Failed to link customer',
		});
	}
}

...
export async function POST(req: MedusaRequest, res: MedusaResponse) {
  try {
    const { email, first_name, last_name, phone } = req.body as FindMatchesRequest;

    if (!email && !first_name && !last_name && !phone) {
      return res.status(400).json({
        message: 'At least one search field (email, first_name, last_name, phone) is required',
      });
    }
    ...
    res.status(500).json({
      message: error instanceof Error ? error.message : 'Failed to find potential matches',
    });
  }
}

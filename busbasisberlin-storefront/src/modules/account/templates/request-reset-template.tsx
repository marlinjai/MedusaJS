...
const RequestResetTemplate = () => {
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			// API Call zur Medusa Backend - Request Reset Password Token
			await sdk.auth.requestPasswordReset({
				identifier: email.trim(),
				provider: 'emailpass', // Standard Provider
			});
			...
			// auch wenn die Email nicht existiert (wie in der Medusa Docs beschrieben)
			setIsSubmitted(true);
		} finally {
			setIsLoading(false);
		}
			...
			<div className="w-full max-w-md mx-auto">
				<div className="bg-green-50 border border-green-200 rounded-lg p-6 text-center">
					...
					<Text className="text-sm text-green-600 mb-6">
						Bitte überprüfen Sie auch Ihren Spam-Ordner. Der Link ist 24 Stunden
						gültig.
					</Text>
